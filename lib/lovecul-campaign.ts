...
// - BL: 50%OFF（割引上限1,500円）、先着、お一人様1度のみ
// - 期限: 2026/05/18 11:59 まで

const FANZA_AFFILIATE_ID = "monodata-995";

// らぶカル BLカテゴリトップ
const LOVECUL_BL_LANDING_URL = "https://lovecul.dmm.co.jp/bl/";
...
// アフィリエイトリダイレクト経由のキャンペーン誘導URL（BL）
// ch=toolbar&ch_id=link は DMM公式ツールバーの正規フォーマット
export function getLoveculCampaignAffiliateUrl(): string {
  return `https://al.fanza.co.jp/?lurl=${encodeURIComponent(LOVECUL_BL_LANDING_URL)}&af_id=${FANZA_AFFILIATE_ID}&ch=toolbar&ch_id=link`;
}
