...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${saleBadge}【${year}年最新】${decodedName}のBL同人ASMR・ゲーム出演作おすすめ${dbWorks.length}選｜声優別レビュー`;
  ...
  const description = `声優「${decodedName}」が出演するBL同人ASMR・ゲーム${dbWorks.length}作品を厳選レビュー。${ratingText}${saleText}${circleText}DLsite・FANZAで人気の${decodedName}出演作の評価・あらすじ・感想を毎日更新。`.slice(0, 160);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        ...
        <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
          声優<span className="font-semibold text-foreground">「{decodedName}」</span>が出演するBL同人ASMR・ゲーム作品
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-base font-bold text-white mb-1" style={{ textShadow: "0 2px 8px rgba(0,0,0,0.9)" }}>
                          {voiceActorFeature.headline || `${decodedName}のおすすめBL作品`}
                        </p>
