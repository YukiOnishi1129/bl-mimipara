...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (dbWorks.length === 0) {
    return { title: "声優が見つかりません | みみぱら" };
  }

  const title = `${decodedName}の出演作品レビュー（${dbWorks.length}作品） | みみぱら`;
  const description = `声優「${decodedName}」のBL ASMR＆ゲーム作品${dbWorks.length}作品を掲載。`;
