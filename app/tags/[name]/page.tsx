...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${saleBadge}【${year}年最新】「${tagName}」のBL同人ASMR・ゲームおすすめ${dbWorks.length}選｜タグ別レビュー`;
  ...
  const description = `「${tagName}」ジャンルのBL同人ASMR・ゲーム${dbWorks.length}作品を厳選レビュー。${ratingText}${saleText}${circleText}DLsite・FANZAで人気の${tagName}作品の評価・あらすじ・感想を毎日更新。`.slice(0, 160);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        ...
        <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
          「<span className="font-semibold text-foreground">{tagName}</span>」ジャンルのBL同人ASMR・ゲーム作品
          ...
          DLsite・FANZAで人気の{tagName}ジャンル作品の評価・あらすじ・感想を毎日更新中。
        </p>
