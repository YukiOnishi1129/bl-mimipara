...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${saleBadge}【${year}年最新】サークル「${decodedName}」のBL同人ASMR・ゲームおすすめ${dbWorks.length}選｜代表作・新作レビュー`;
  ...
  const description = `サークル「${decodedName}」のBL同人ASMR・ゲーム${dbWorks.length}作品を厳選レビュー。${ratingText}${saleText}${actorText}DLsite・FANZAで人気の${decodedName}作品の評価・あらすじ・感想を毎日更新。`.slice(0, 160);
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        ...
        <div className="mb-8 rounded-2xl border border-border bg-card p-6">
          <h1 className="text-2xl font-bold text-foreground font-heading">
            {circle.name}のBL同人ASMR・ゲームおすすめ{totalCount}選
          </h1>
          ...
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            サークル<span className="font-semibold text-foreground">「{circle.name}」</span>のBL同人ASMR・ゲーム作品
