...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `【${year}年最新】${feature.name}特集 BL同人ASMR・ゲームおすすめ${totalRecommended}選`;
  const baseDesc =
    feature.description ||
    `「${feature.name}」ジャンルの人気BL同人ASMR・ゲーム作品を厳選レビュー。迷ったらここから選べばハズレなし。`;
...
export default async function SeihekiTokushuPage({ params }: Props) {
  ...
  // match_tagsの最初のタグで関連作品を取得
  const primaryTag = feature.match_tags?.[0] || feature.name;
  ...
  const otherSeihekiFeatures = allSeihekiFeatures
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-4">
        <Breadcrumb
          items={[
            { label: "トップ", href: "/" },
            { label: "ジャンル特集", href: "/tokushu/seiheki" },
            { label: feature.name },
          ]}
        />
        ...
        <div className="mb-6 mt-4">
          ...
          <div className="p-4 bg-linear-to-r from-primary/10 to-accent/10 rounded-lg border border-primary/20">
            ...
            <p className="text-base font-bold text-foreground">
              {feature.headline ||
                `「${feature.name}」のBL作品、厳選おすすめ`}
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              {feature.description ||
                `「${feature.name}」ジャンルの作品から迷ったらこれを選べ。`}
            </p>
            {feature.posted_at && (
        ...
        <div className="mt-8 mb-10">
          <Link href={`/tags/${encodeURIComponent(tagNameToSlug(primaryTag))}`}>
                ...
                >
                  <Card className="overflow-hidden border border-border hover:border-primary/50 transition-all">
                    <div className="relative aspect-[16/9] overflow-hidden bg-muted">
                      <img
                        src={
                          seiheki.representative_thumbnail_url ||
                          "https://placehold.co/200x112/f0eef8/8b7d9e?text=BL"
                        }
                        alt={seiheki.name}
                        loading="lazy"
                        className="h-full w-full object-cover"
